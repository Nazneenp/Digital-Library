...
if (loginForm) {
    ...
    loginForm.addEventListener('submit', (e) => {
        ...
        if (loginName.value != 'admin' && loginName.value != 'admin@gmail.com') {
            logMsg.push('Error!')
            logErrorMsg.textContent = 'Error! User Name is wrong.'
        } else if (loginPassword.value != 'admin123') {
            logMsg.push('Error!')
    ...
    if (sessionStorage.getItem('logSuccess')) {
        ...
        if (logSuccessName) {
            ...
            loginUserName[i].textContent = 'Successfully Logged in as Admin'
        }
...
if (frgtForm) {
    frgtForm.addEventListener('submit', (e) => {
        ...
        if (frgtEmail.value != 'admin@gmail.com') {
            frgtMsg.push('Error!')
